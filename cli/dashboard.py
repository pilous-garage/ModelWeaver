...
def cmd_agents(args):
    ...
    agents = data.get("agents") or data.get("result", {}).get("agents", [])
    if not agents:
    ...
    _print_table("Agents", ("ID", "Nom", "Rôle", "Status", "Actif", "Heartbeat", "Step"), rows)
...
def main():
    ...
    p_agents = sub.add_parser("agents", help="Lister les agents")
    p_agents.set_defaults(func=cmd_agents)

    p_services = sub.add_parser("services", help="Lister / piloter les services")
    p_services.add_argument("service_action", nargs="?", choices=("restart", "stop"), help="Action sur un service")
