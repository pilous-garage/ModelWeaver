...
def op_service_register(params):
    """Enregistre un agent comme service supervisé (agent-as-service)."""
    name = params.get("name")
    agent_id = params.get("agent_id")
    ...
    try:
        ...
        conn.commit()
        conn.close()
        # Route dynamique pour interroger ce service
...
def op_service_unregister(params):
    """Désenregistre un agent-as-service (stop + remove)."""
    name = params.get("name")
    if not name:
        return {"status": "error", "error": "name requis"}
    import sqlite3, signal, os as _os
    try:
        db = runtime_db_path()
        conn = sqlite3.connect(str(db))
