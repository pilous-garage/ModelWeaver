...
function dispatchMouse(el: Element, type: string, x: number, y: number): boolean {
  const opts: any = { bubbles: true, cancelable: true, clientX: x, clientY: y, button: 0 };
  const ev = new MouseEvent(type, opts);
  return el.dispatchEvent(ev);
}

/** Simule un clic à (x,y) ou sur un élément ciblé. */
function doClick(params: any): { ok: boolean; info: string } {
  ...
  return { ok: true, info: `click @(${cx},${cy}) sur <${el.tagName.toLowerCase()}${el.id ? '#' + el.id : ''}>`, clicked };
}
...
function doType(params: any): { ok: boolean; info: string } {
  ...
  if (typeof input.value === 'string' && typeof input.setNativeValue !== 'function') {
...
// Un SEUL poller global pour toutes les fenêtres (sinon 3 webviews pollent
// en parallèle et épuisent le quota du daemon). Le flag est partagé via
// window pour traverser les webviews de la même app.
 ...
 * résultats. Un seul poller global (la première webview le démarre). */
export async function startGuiInspectorPoll(intervalMs = 1500): Promise<void> {
  ...
  logGui('gui:inspector-start', {});
  // Marqueur visible immédiat (avant tout await) pour le debug.
  ...
  const tick = async () => {
    try {
      ...
      try { await daemonPost('logs/write', { level: 'GUI', message: '[full-log] guiInspector-poll-err: ' + String(e?.message || e).slice(0, 120) }); } catch {}
    }
  };
  // Premier poll immédiat (fiable même si les timers de la webview sont
  // throttlés en arrière-plan), puis boucle périodique.
  await tick();
  const loop = async () => {
    while (_isPolling()) {
