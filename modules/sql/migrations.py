...
Contient également les helpers de migration et les classes d'application
par base de données, extraits de db.py.
"""
...
# ──────────────────────────────────────────────
#  Migrations par base de données
# ──────────────────────────────────────────────

class ModelWeaverMigrations:
    """Migrations de la base locale modelweaver.db."""

    def __init__(self):
        self._sql_dir = Path(__file__).resolve().parent

    def ensure_schema(self, conn: sqlite3.Connection) -> None:
        """Crée/met à jour le schéma de modelweaver.db."""
        schema = self._sql_dir / "modelweaver_schema.sql"
        if schema.exists():
...
class CatalogueMigrations:
    """Migrations de la base catalogue.db."""

    def __init__(self):
        self._sql_dir = Path(__file__).resolve().parent

    def ensure_schema(self, conn: sqlite3.Connection) -> None:
        """Crée/met à jour le schéma de catalogue.db."""
        schema = self._sql_dir / "catalogue_schema.sql"
        cur = conn.execute(
...
class RuntimeMigrations:
    """Migrations de la base runtime.db."""

    def __init__(self):
        self._sql_dir = Path(__file__).resolve().parent

    def ensure_schema(self, conn: sqlite3.Connection) -> None:
        """Crée/met à jour le schéma de runtime.db."""
        conn.execute("""
...
class AgentsMigrations:
    """Migrations de la base agents.db."""

    def __init__(self):
        self._sql_dir = Path(__file__).resolve().parent

    def ensure_schema(self, conn: sqlite3.Connection) -> None:
        """Crée/met à jour le schéma de agents.db."""
        schema = self._sql_dir / "agents_schema.sql"
        if schema.exists():
            conn.executescript(schema.read_text())
