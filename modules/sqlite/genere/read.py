...
def list_backups(db: Db, project_id: int, backup_of: str) -> List[Dict[str, Any]]:
    return db.table("gen_data").select(where={"project_id": project_id, "backup_of": backup_of})
...
def get_run(db: Db, project_id: int, data_ref: str, run_seq: int) -> Optional[Dict[str, Any]]:
    return db.table("gen_runs").get({"project_id": project_id, "data_ref": data_ref, "run_seq": run_seq})


def list_runs(db: Db, project_id: int, data_ref: str) -> List[Dict[str, Any]]:
...
def get_dependencies(db: Db, project_id: int, data_ref: str) -> List[Dict[str, Any]]:
...
def runtime_globs(db: Db, project_id: int, data_ref: str) -> List[Dict[str, Any]]:
