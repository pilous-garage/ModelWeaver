...
def install_system_package(package_name: str) -> dict:
    """Installe un paquet via apt (update des listes inclus).

    Fonctionne sans terminal (pas de prompt interactif) :
    - root          : apt direct
    ...
    try:
        ...
        res = subprocess.run(pre + ["apt-get", "install", "-y", package_name],
