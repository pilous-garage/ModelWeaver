          ...
          {showChat ? (
            ...
            <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: '1rem', overflow: 'hidden' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexShrink: 0 }}>
                ...
                <button onClick={fetchLocalEngines} disabled={localLoading}
                  style={{ padding: '0.3rem 0.7rem', fontSize: '0.72rem', backgroundColor: localLoading ? '#1e293b' : '#2563eb', color: 'white', border: 'none', borderRadius: '0.3rem', cursor: localLoading ? 'default' : 'pointer' }}>
                  {localLoading ? <Spinner size={12} color="#64748b" /> : '↻ Actualiser'}
                </button>
              </div>
              {localMsg && <div style={{ color: '#fca5a5', fontSize: '0.75rem' }}>{localMsg}</div>}
              <div style={{ flex: 1, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: '0.7rem' }}>
                {localEngines.length === 0 && !localLoading && (
                  ...
                  <div key={e.ref} style={{ backgroundColor: '#1e293b', borderRadius: '0.5rem', border: '1px solid #334155', padding: '0.8rem' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                        ...
                        <button
                          onClick={() => handleLocalToggle(e.ref, e.running)}
                          disabled={localBusy === e.ref || (e.running === false && !e.headless)}
                          style={{ padding: '0.3rem 0.7rem', fontSize: '0.72rem', fontWeight: '600', borderRadius: '0.3rem', border: 'none', cursor: (localBusy === e.ref || (e.running === false && !e.headless)) ? 'default' : 'pointer', backgroundColor: e.running ? '#7f1d1d' : '#059669', color: e.running ? '#fecaca' : 'white' }}
                        >
                          {localBusy === e.ref ? <Spinner size={12} color="#94a3b8" /> : e.running ? 'Arrêter' : 'Démarrer'}
                        </button>
                      </div>
                    </div>
